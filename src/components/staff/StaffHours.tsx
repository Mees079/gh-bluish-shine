    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="bg-[#0e1524]/60 border border-[#25303f] rounded-xl overflow-hidden">
            <div className="grid grid-cols-[1fr_80px_80px] gap-2 px-4 py-3 text-xs font-medium text-[#9ba7ba] uppercase tracking-wider border-b border-[#25303f]">
              <span>Naam</span>
            ...
            {hours.map(entry => {
              ...
              const required = getRequiredHours(name);
              return (
                <div key={entry.id} className="px-4 py-3 border-b border-[#25303f]/50 last:border-0">
                  <div className="grid grid-cols-[1fr_80px_80px] gap-2 items-center">
                    <span className="text-sm text-white font-medium">{name}</span>
                    <span className={`text-sm text-center ${entry.notes === 'AFGEMELD' ? 'text-[#3a465c]' : 'text-white'}`}>
                      {entry.notes === 'AFGEMELD' ? '-' : fmtH(entry.hours)}
                    </span>
                    <span className={`text-xs text-center px-2 py-1 rounded ${entry.notes === 'AFGEMELD' ? 'bg-red-500/10 text-red-400' : 'bg-[#337aff]/10 text-[#337aff]'}`}>
                  ...
                  {entry.notes === 'AFGEMELD' && (
                    <p className="text-[10px] text-[#9ba7ba] mt-1.5">Moet deze week alsnog {required.toFixed(1).replace('.', ',')} uur halen</p>
                  )}
                </div>
              );
            })}
            <div className="flex justify-between items-center px-4 py-3 bg-[#25303f]/30">
              <span className="text-sm font-medium text-[#9ba7ba]">Totaal uren</span>
              <span className="text-sm font-bold text-[#337aff]">{fmtH(totalHours)}</span>
            </div>
          </div>
